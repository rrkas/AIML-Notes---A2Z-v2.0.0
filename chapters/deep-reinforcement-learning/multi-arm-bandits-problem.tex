...
\[
    Q_t(a) = \displaystyle\dfrac{\text{sum of rewards when $a$ taken prior to $t$}}{\text{number of times $a$ taken prior to $t$}} = \displaystyle\dfrac{\displaystyle\sum_{i=1}^{t-1} R_i \cdot \mathbbm{1}_{A_i = a}}{\displaystyle\sum_{i=1}^{t-1} \mathbbm{1}_{A_i = a}}
\]
...
\begin{align*}    
    ...
    &= \displaystyle\dfrac{1}{n} \left( R_n + (n-1) \displaystyle\dfrac{1}{(n-1)}  \left(\displaystyle\sum_{i=1}^{n-1} R_i\right) \right)\\
...
\[
    \pi_t(a) = Pr\{A_t = a\} = P(A_t=a) = \displaystyle\dfrac{e^{H_t(a)}}{\displaystyle\sum_{i=1}^{k} e^{H_t(i)}}
\]
