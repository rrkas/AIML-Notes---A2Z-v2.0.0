...
\section{Chain Rule of Conditional Probabilities \cite{dnn-deep-learning-ian}}\label{Chain Rule of Conditional Probabilities}

\[
    P\dParenBrac{x^{(1)},\cdots,x^{(n)}}
    = P\dParenBrac{x^{(1)}} \dprod_{i=2}^n P\dParenBrac{x^{(i)}|x^{(1)},\cdots,x^{(i-1)}}
\]
\[
    \begin{aligned}
        P(a,b,c)
...
\begin{enumerate}
    ...
    \item $
        \mathbb{E}_x[\alpha f(x) + \beta g(x)]
        = \alpha\mathbb{E}_x[f(x)] + \beta\mathbb{E}_x[g(x)]
    $ \hfill \cite{dnn-deep-learning-ian}
\end{enumerate}
...
\begin{enumerate}[itemsep=0.2cm]
    \item $Var(f(x)) 
    = \mathbb{E}\dSquareBrac{(f(x) - \mathbb{E}[f(x)])^2}$ \hfill \cite{dnn-deep-learning-ian}

    \item The variance of a random variable $X$ with states $x \in R^D$ and a mean vector $\mu \in R^D$ is defined as $\mu = \mathbb{E}_X(x)$
...
\begin{enumerate}[itemsep=0.2cm]
    ...
    If the sign of the covariance is \textbf{positive}, then both variables tend to take on relatively high values simultaneously.\\
    If the sign of the covariance is \textbf{negative}, then one variable tends to take on a relatively high value at the times that the other takes on a relatively low value and vice versa. \cite{dnn-deep-learning-ian}

    \item The notions of covariance and dependence are related, but are in fact distinct concepts.\\
    ...
    \textbf{It is possible for two variables to be dependent but have zero covariance.} \hfill \cite{dnn-deep-learning-ian}\\
    \textbf{Example}: Consider uniform distribution over the interval $[-1, 1]$ and $3$ random variables, such that $s \sim \mathcal{U}(-1,1)$ \textbf{and} $y=sx$\\
    $x$ and $y$ are not independent, because $x$ completely determines the magnitude of $y$. However, $Cov(x, y) = 0$.\\
    \textbf{Proof}:\\
    \[
        \mathbb{E}[s] = \int_{-1}^1 s \cdot \frac{1}{2} \, ds = 0
        \hspace{2cm}
        \mathbb{E}[x] = \int_{-1}^1 x \cdot \frac{1}{2} \, dx = 0
        \hspace{2cm}
        \mathbb{E}[y] = \mathbb{E}[s x] = \mathbb{E}[s] \cdot \mathbb{E}[x] = 0 \cdot 0 = 0
    \]
    \[
        \mathbb{E}[xy] 
        =\mathbb{E}[sx^2]
        = \mathbb{E}[s] \cdot \mathbb{E}[x^2]
        = 0 \cdot \mathbb{E}[x^2] = 0
    \]
    \[
        Cov(x, y) 
        = Cov(x, sx)
...
\section{Correlation \cite{dnn-deep-learning-ian}}\label{Multivariate Distributions: Correlation}

\begin{enumerate}
    \item correlation normalize the contribution of each variable in order to measure only how much the variables are related, rather than also being affected by the scale of the separate variables.
\end{enumerate}
