...
\begin{itemize}[itemsep=0.1cm]
    ...
    \item \(c_{KN}(\cdot) = \begin{cases}
        \operatorname{count}(\cdot) & \text{for the highest order} \\
        \operatorname{continuationcount}(\cdot) & \text{for lower orders}
    \end{cases}\)
    \item $\operatorname{continuationcount}(x)$ of a string $x$ is the number of unique single word contexts for that string $x$.
