...
For bigram:
\begin{align*}
    P_{KN}(w_i|w_{i-1})  &= \displaystyle\frac{max(C(w_{i-1}w_i)-d,0)}{C(w_{i-1})} \\ 
...
\begin{align*}
    P_{KN}(w_i|w_{i-n+1:i-1})  &= \displaystyle\frac{max(c_{KN}(w_{i-n+1:i})-d,0)}{\sum_v c_{KN}(w_{i-n+1:i-1} v)} \\ 
