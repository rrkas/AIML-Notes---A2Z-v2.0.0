...
\begin{itemize}
    ...
    \item A \textbf{one-hot vector}\index{one-hot vector}\label{one-hot vector} is a vector that has one element equal to $1$ - in the dimension corresponding to that word’s index in the vocabulary - while all the other elements are set to $0$.
...
\begin{table}[H]
    ...
    \begin{minipage}{0.65\linewidth}
        \begin{itemize}
            ...
            \item We then multiply these one-hot vectors by the \textbf{embedding matrix}\index{embedding matrix} $\mathbf{E}$.

            \item The \textbf{embedding weight matrix}\index{embedding weight matrix}\label{embedding weight matrix} $\mathbf{E}$ has a column for each word, each a column vector of $d$ dimensions, and hence has dimensionality $d \times |V|$. 
...
\begin{enumerate}
    ...
    \item \textbf{Multiply by} $\mathbf{W}$: We multiply by $\mathbf{W}$ (and add $\mathbf{b}$) and pass through the ReLU (or other) activation function (SEE: \nameref{chapter: Activation Functions}) to get the hidden layer $\mathbf{h}$.
    \begin{center}
