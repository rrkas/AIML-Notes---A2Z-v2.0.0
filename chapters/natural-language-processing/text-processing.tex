...
Like mathematical or logical operations, a morphosyntactic operation will typically have a former operator, such as a \textbf{suffix} \index{morphosyntactic operation: suffix} (like -s in bugs) or a \textbf{prefix} \index{morphosyntactic operation: prefix} (like re- in reread), collectively called \textbf{affixes}, that operate in a syntactic way on the base form (like bug \& read).
...
\begin{itemize}
    \item A \textbf{simple sentence}\index{simple sentence} has only one clause, and one independent variable.\\ \textbf{Example}: The cat is sleeping.
    \item A \textbf{compound sentence}\index{compound sentence} has two or more clauses. These clauses are joined with conjunctions, punctuation, or both.\\ \textbf{Example}: The dog is happy, but the cat is sad.
    \item A \textbf{complex sentence}\index{complex sentence} has one clause with a relative clause. \\ \textbf{Example}: The dog, which is eating the bone, is happy.
...
\section{Syntax}\label{Language: Syntax}
    \textbf{Syntax} is the study of phrases and clauses, or of the sentence structure.\\ Syntax comes from the \textit{Greek} \textbf{suntaxis}\index{suntaxis} meaning 'arrange together'.
...
\section{Syntactic operations}\label{Syntactic operations}
    ...
    Example: tense (or aspect) \index{Syntactic operations: tense/ aspect} \label{Syntactic operations: aspect/ tense} is a necessary operator within a clause that must interact with the noun and verb in order for the components to form a licit sentence.
