...
\begin{enumerate}
    ...
    \[
        \displaystyle \rcmdXPPMI_\alpha(w, c) = \max\left(\log_2\left(\frac{P(w, c)}{P(w)P_\alpha(c)}\right),0\right)
    \]    
...
\[
    P_\alpha(w) = \frac{\rcmdXcount(w)^\alpha}{\displaystyle\sum_{w'} \rcmdXcount(w')^\alpha}
\]
