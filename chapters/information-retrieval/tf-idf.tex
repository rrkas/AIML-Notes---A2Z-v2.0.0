...
\begin{itemize}
    \item In a \textbf{term-document matrix}\indexlabel{term-document matrix}, each row represents a word in the vocabulary and each column represents a document from some collection of documents.
...
\begin{itemize}
    \item \textbf{Co-occurrence matrix/ term-term matrix/ word-word matrix/ term-context matrix}\indexlabel{Co-occurrence matrix} \indexlabel{term-term matrix} \indexlabel{word-word matrix} \indexlabel{term-context matrix}, in which the columns are labeled by words rather than documents. 
